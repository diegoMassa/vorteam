...
public class S3Services implements IS3Services {
	 ...
	 * @throws Exception
	 */
	public String uploadPublicRead(String bucketName, String key, Byte[] bytes) throws Exception {
		log.info("Upload file:"+key+"			bucketName:" + bucketName + "\n");
		AmazonS3 amazonS3 = buildAmazonS3(); 
		 try {
			 ...
			 byte[] bytesPrimitivos = new byte[bytes.length];
			 for (int i=0; i<bytes.length; i++){
				 bytesPrimitivos[i] = bytes[i];
			 }
			 
			 ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bytesPrimitivos);
			 
			 amazonS3.putObject(new PutObjectRequest(bucketName, key, byteArrayInputStream, objectMetadata).withCannedAcl(CannedAccessControlList.PublicRead));			 
			 return getUrl(bucketName, key);
		} catch (Exception e) {
