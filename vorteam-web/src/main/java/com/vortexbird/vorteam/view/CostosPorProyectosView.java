...
@ManagedBean
@ViewScoped
public class CostosPorProyectosView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(CostosPorProyectosView.class);
	...
	public void buscarProyectos() {
		...
		if(!FacesUtils.checkString(somEstadoProyecto).equals(Constantes.SOM_OPCION_SELECCIONAR.toString())) {
			...
			List<VtProyectoDTO> proyectoDTOs = businessDelegatorView.consultaProyectosClientes(FacesUtils.checkString(somEstadoProyecto));
	...
	/**
	 *
	 * @param usuario_session El/La usuario_session a setear
	 * @author Daniel Pareja Londoño
	 * @version sept. 20, 2018
	 * @since 1.8
	 *
	...
	/**
	 *
	 * @param calFechaInicial El/La calFechaInicial a setear
	 * @author Daniel Pareja Londoño
	 * @version sept. 20, 2018
	 * @since 1.8
	 *
	...
	/**
	 *
	 * @param calFechaFinal El/La calFechaFinal a setear
	 * @author Daniel Pareja Londoño
	 * @version sept. 20, 2018
	 * @since 1.8
	 *
	...
	public List<SelectItem> getSiPersonas() {
		try {
			if(siPersonas == null) {
				...
				Object[] variablesBusquedaPersonas = {
						"activo",true,Constantes.ESTADO_ACTIVO,"="
				};
				List<VtPersonas> personas = businessDelegatorView.findByCriteriaInVtPersonas(variablesBusquedaPersonas, null, null);
	...
	/**
	 *
	 * @param siPersonas El/La siPersonas a setear
	 * @author Daniel Pareja Londoño
	 * @version sept. 20, 2018
	 * @since 1.8
	 *
	...
	/**
	 *
	 * @param siTiposActividad El/La siTiposActividad a setear
	 * @author Daniel Pareja Londoño
	 * @version sept. 20, 2018
	 * @since 1.8
	 *
	...
	/**
	 *
	 * @param somPersona El/La somPersona a setear
	 * @author Daniel Pareja Londoño
	 * @version sept. 20, 2018
	 * @since 1.8
	 *
	...
	/**
	 *
	 * @param somProyecto El/La somProyecto a setear
	 * @author Daniel Pareja Londoño
	 * @version sept. 20, 2018
	 * @since 1.8
	 *
	...
	public void cargarDatosParaInforme() {
		...
		persId = FacesUtils.checkLong(somPersona).equals(Constantes.SOM_OPCION_SELECCIONAR)?null:FacesUtils.checkLong(somPersona);
		proyId = FacesUtils.checkLong(somProyecto).equals(Constantes.SOM_OPCION_SELECCIONAR)?null:FacesUtils.checkLong(somProyecto);
		tiacId = FacesUtils.checkLong(somTipoActividad).equals(Constantes.SOM_OPCION_SELECCIONAR)?null:FacesUtils.checkLong(somTipoActividad);
	}
	
	public StreamedContent getReporteXlsx() {
		try {
			cargarDatosParaInforme();
			ByteArrayInputStream bais = businessDelegatorView.generarReporteCostosPorProyectos(fechaInicio, fechaFin, persId, proyId, tiacId, Constantes.REPORT_OUTPUT_TYPE.XLSX);
			fileReporteGenerado = new DefaultStreamedContent(bais, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "CostosPorProyectos.xlsx");
	...
	public StreamedContent getReporteXls() {
		try {
			cargarDatosParaInforme();
			ByteArrayInputStream bais = businessDelegatorView.generarReporteCostosPorProyectos(fechaInicio, fechaFin, persId, proyId, tiacId, Constantes.REPORT_OUTPUT_TYPE.XLS);
			fileReporteGenerado = new DefaultStreamedContent(bais, "application/vnd.ms-excel",
					"CostosPorProyectos.xls");
			return fileReporteGenerado;
		} catch (Exception e) {
			FacesUtils.addErrorMessage(e.getMessage());
			return null;
	...
	public StreamedContent getReportePdf() {
		try {
			cargarDatosParaInforme();
			ByteArrayInputStream bais = businessDelegatorView.generarReporteCostosPorProyectos(fechaInicio, fechaFin, persId, proyId, tiacId, Constantes.REPORT_OUTPUT_TYPE.PDF);
			fileReporteGenerado = new DefaultStreamedContent(bais, "application/pdf",
					"CostosPorProyectos.pdf");
			return fileReporteGenerado;
		} catch (Exception e) {
			FacesUtils.addErrorMessage(e.getMessage());
			return null;
