...
@ManagedBean
@ViewScoped
public class OcupacionPlantaView implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LoggerFactory.getLogger(OcupacionPlantaView.class);
	...
	public StreamedContent getReporteXlsx() {
		try {
			...
			ByteArrayInputStream bais = businessDelegatorView.generarReporteOcupacionPlanta(fechaInicio, fechaFin,
					persId, proyId, tiacId, Constantes.REPORT_OUTPUT_TYPE.XLSX);
			fileReporteGenerado = new DefaultStreamedContent(bais,
					"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "OcupacionPlanta.xlsx");
	...
	public StreamedContent getReporteXls() {
		try {
			...
			fileReporteGenerado = new DefaultStreamedContent(bais, "application/vnd.ms-excel", "OcupacionPlanta.xls");
			return fileReporteGenerado;
		} catch (Exception e) {
			FacesUtils.addErrorMessage(e.getMessage());
			return null;
	...
	public StreamedContent getReportePdf() {
		try {
			...
			fileReporteGenerado = new DefaultStreamedContent(bais, "application/pdf", "OcupacionPlanta.pdf");
			return fileReporteGenerado;
		} catch (Exception e) {
			FacesUtils.addErrorMessage(e.getMessage());
			return null;
